...
- **Maximum supply limit**: Rune cannot exceed 9e15 base units (9 quadrillion base units) to ensure precision in financial calculations
...
<Note>
  ...
  **Maximum Supply Calculator**: To calculate if your Rune meets the platform limits:
  - Base Units = Token Supply × 10^Divisibility
  - Must be: 50,000,000 < Base Units ≤ 9,000,000,000,000,000 (9e15)
  - Example: 90M tokens × 10^8 divisibility = 9e15 base units ✅ Maximum allowed
  - Example: 100M tokens × 10^8 divisibility = 10e16 base units ❌ Exceeds limit
</Note>
...
- **Fixed Supply**: All etched Runes have a fixed supply of 90 million tokens with 8 divisibility (9e15 base units - safely within JavaScript precision limits)
...
### Token Distribution (100% Supply)

- **80% Investors**: 10% instant + 70% vested over ~60 days
- **10% Liquidity Pool**: Paired with 75% of BTC raised
- **10% DAO**: Paired with 20% of BTC raised

### BTC Distribution

- **75%** → Liquidity Pool (enables instant trading)
- **20%** → DAO Treasury (community governance)
- **5%** → Platform Fee (only on successful presales)

### Timeline

1. Presale completes → UTXO Split Migration (~10 min) → Distribution → Confirmation → Live trading
2. Full process: **20-90 minutes** depending on network congestion

<Info>
  **Trading can go live before distribution completes\!** Once the migration transaction is confirmed on-chain, the token becomes tradeable even if investor distributions are still processing in the background. This means you may be able to trade/swap while migration is still in progress.
</Info>

<Note>
  This distribution is hardcoded and identical for all tokens, ensuring fairness. No team allocations, no insider tokens, no pre-mines.
</Note>
...
<Accordion title="Can I use a Rune I don't completely own?">
  No, you must own 100% of the Rune's supply to launch a token. Additionally, the Rune must have more than 50 million base units and cannot exceed 9e15 base units (total supply × 10^divisibility). The total supply must also be divisible by 100. This ensures fair launches, prevents conflicts between multiple token creators, and maintains mathematical precision in our platform's financial calculations.
</Accordion>
